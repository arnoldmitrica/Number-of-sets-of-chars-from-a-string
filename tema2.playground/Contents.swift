...
func maximumNumberOfMoves(sampleWord word:String, appliedTo str: String) -> Any {

    var setWord = Set<Character>()

    return zip(word.filter{ setWord.insert($0).inserted }.sorted().map { x in word.filter{ $0 == x}.count }.reversed() as [Int], (setWord.sorted().reversed() as [Character]).map { x in str.filter { $0 == x}.count }.filter { $0 != 0 } as [Int]).map{$1/$0}.min() ?? "Nil unrwapped" as Any
